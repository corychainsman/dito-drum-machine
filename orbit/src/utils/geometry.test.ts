...
describe('geometry', () => {
  it('generateAllArcs returns exactly 40 arcs', () => {
  ...
  it('hit-test at 12 o\'clock on innermost ring returns { ring: 4, step: 0 }', () => {
    expect(hitTest(200, 200 - 63)).toEqual({ ring: 4, step: 0 });
  });

  it('all 40 centroids are within the SVG bounds (0–400)', () => {
    generateAllArcs().forEach(arc => {
